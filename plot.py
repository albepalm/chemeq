...
paths = [os.path.join(dp, f) for dp, dn, filenames in os.walk('plt_data_map')\
            for f in filenames if os.path.splitext(f)[1] == '.dat']
#paths.sort()
pressVec = np.array([float(mystr[9+4:20+4]) for mystr in paths])
idx = np.argsort(pressVec)
pressVecS = pressVec[idx]
#print(pressVec[idx])
pathsS = [paths[i] for i in idx]
#print(pathsS)
