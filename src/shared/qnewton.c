		...
		int output)
{
	...
	do
	{
		...
		for(uint64_t j = 0; j < d; ++j)
		{
			...
			{
				LINALG_COPY_VEC(&xf,x);
				LINALG_COPY_VEC(&xb,x);
				(xf).pt[k] += epsJ;
				(xb).pt[k] -= epsJ;
				jacobian.pt[j*d+k] = 
					(F[j]((xf).pt, (*fpar).pt)-F[j]((xb).pt, (*fpar).pt))
					/(2*epsJ);
			}
		}
		...
		if(is_maxit_reached || is_tol_reached)
		{
			...
			if(output >= 1 )
			{
				...
				LINALG_PRINT_VEC(x,"Solution");
				LINALG_PRINT_VEC(&neg_system,"Function");
				LINALG_PRINT_MAT(&jacobian,"Jacobian II ord. approx.");
		...
		{
			++i;
			LINALG_SOLVE(&y,&jacobian,&neg_system,d);
	...
	LINALG_DEALLOC_VEC(&xf);
	LINALG_DEALLOC_VEC(&xb);
	return;
}
